...
* **v3.23.0** (2026.05) — Tibetan/Khmer/Lao/Burmese letter→IPA fallback
  ...
  * 추가된 letter→IPA 매핑:
    * **Tibetan** (티베트, U+0F00..U+0FFF) — 30+ base consonants + subjoined consonants + tsek 처리
    * **Khmer** (크메르, U+1780..U+17FF) — 35+ consonants + 다이어크리틱 + coeng (្) 처리
    * **Lao** (라오, U+0E80..U+0EFF) — 25+ consonants + 결합 모음 + 톤 마크 silent 처리
    * **Burmese** (버마/미얀마, U+1000..U+109F) — 30+ consonants + 모음 마크 + asat (်) 처리
  * `_detect_script()` Unicode block 분류 4개 추가
  * `transcribe_auto`에 라우팅 분기 추가 — strict mode leak 0 보장
  * 검증 예시: བོད (티베트) → 볻, ខ្មែរ (크메르) → 크멜, ວຽງຈັນ (비엔티안) → 위앙찬, မြန်မာ (미얀마) → 므은마
  * 한계: 복잡한 syllable structure는 letter-level rough mapping이며, 정밀 G2P는 별도 신경망 필요
  * 전체 테스트: **379 passed**
